...
static void ovl_free_fs(struct ovl_fs *ofs)
{
	...
	ovl_ctrl_proc_prober(false);
	printk("[ovl_exit] GC probe deleted by %d.", current->pid);

	/*destroy cached table */
	ovl_destroy_cached_table(ofs->cached_table);
	/* */
...
static int ovl_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	/* init cached table */
	ofs->cached_table = kzalloc(sizeof(struct cached_tbl), GFP_KERNEL);
	ovl_init_cached_table(ofs->cached_table); // init static addr for probe
	
	printk("[ovl_fill_super] cached_table created. addr: %p, creator: %d", ofs->cached_table, current->pid);
	
	ovl_ctrl_proc_prober(true);
	printk("[ovl_fill_super] proc_prober created by %d.", current->pid);
	ofs->cached_table->sb = sb;
