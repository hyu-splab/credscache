...
static struct cached_tbl *cached_table_static;

void ovl_init_cached_table(struct cached_tbl *cached_table) 
{
	
	hash_init(cached_table->table);
	mutex_init(&cached_table->lock);
	cached_table->gc = false;

	cached_table_static = cached_table; // for probe
...
static void proc_prober(void *data, struct task_struct *task)
{
	pid_t pid;
	pid = task->pid;
	ovl_gc_cached_creds(cached_table_static, pid);
}

void ovl_ctrl_proc_prober(bool enable){
	char* probe_name = kmalloc(sizeof(char) * 30, GFP_KERNEL);
	struct trace_array *tr;
	
	sprintf(probe_name, "ovl-tracer-%p", cached_table_static);
	tr = trace_array_get_by_name(probe_name);
	if(enable) {
		printk("[ovl_ctrl_proc_prober] probe created - %s", probe_name);
		register_trace_sched_process_exit(proc_prober, tr);
	} else {
		printk("[ovl_ctrl_proc_prober] probe destroyed - %s", probe_name);
		unregister_trace_sched_process_exit(proc_prober, tr);
...
int ovl_gc_get_table_size(struct cached_tbl *cached_table) 
{
	struct cached_node *cur_data;
	unsigned bkt;
...
void ovl_gc_cached_creds(struct cached_tbl *cached_table, pid_t pid)
{
	const struct cred* cached_cred;
	struct ovl_fs *ofs = cached_table->sb->s_fs_info;
	const struct cred *creator_cred = ofs->creator_cred;
	unsigned bkt;
	struct hlist_node *tmp;
	struct cached_node *cur_data;
	pid_t pid_found;
	...
	hash_for_each_safe(cached_table->table, bkt, tmp, cur_data, node) {
		...
		if (pid == pid_found) {
			...
			hash_del(&cur_data->node);
			if (cur_data) kvfree(cur_data);
			printk("[GC done!] pid : %d deleted", pid_found);
			break;
		}
	}
	cached_table->gc = false;
...
void ovl_destroy_cached_table(struct cached_tbl *cached_table)
{
	unsigned bkt;
	struct hlist_node *tmp;
	struct cached_node *cur_data;
	...
	printk("[ovl_destroy_cached_table] %d cached cred deleted!", cnt);
}

void ovl_gc_cached_creds_active(struct super_block *sb) 
{
	struct ovl_fs *ofs = sb->s_fs_info;
	const struct cred* creator_cred = ofs->creator_cred;
	struct cached_node *cur_data;
	struct cached_tbl *cached_table = ofs->cached_table;
	...
	hash_for_each_safe(cached_table->table, bkt, tmp, cur_data, node) {
		task = NULL;
		pid = (pid_t) cur_data->pid;
		pid_str = find_get_pid(pid);
		task = get_pid_task(pid_str, PIDTYPE_PID);
		if (!task) {
			hash_del(&cur_data->node);
			put_cred(creator_cred);
			if (cur_data) kvfree(cur_data);
...
const struct cred *ovl_get_cached_creds(struct cached_tbl *cached_table, __u32 pid, bool new) 
{
	struct cached_node *cur_data;
	struct hlist_node *tmp;
	const struct cred *cached_cred = NULL;
	struct ovl_fs *ofs = cached_table->sb->s_fs_info;
	
	//unsigned long flags;

	hash_for_each_possible_safe(cached_table->table, cur_data, tmp, node, pid) {	
		if (cur_data->pid == pid) {
			cached_cred = cur_data->cached_cred;
			return cached_cred;
		}
	}

	if (cached_cred == NULL && new) {
		struct cached_node *temp_node;
		temp_node = kzalloc(sizeof(struct cached_node), GFP_KERNEL);
		temp_node->cached_cred = kzalloc(sizeof(struct cred), GFP_KERNEL);
		memcpy(temp_node->cached_cred, ofs->creator_cred, sizeof(struct cred));
		temp_node->cached_cred->non_rcu = 0;
...
const struct cred *ovl_override_cached_creds(struct super_block *sb)
{
	struct ovl_fs *ofs = sb->s_fs_info;
	const struct cred *old_cred = current->cred;
	__u32 pid = current->pid;
	const struct cred* cached_cred = ovl_get_cached_creds(ofs->cached_table, pid, true);

	validate_creds(cached_cred);
	validate_creds(old_cred);
	
	current->cred = (struct cred *)cached_cred;

	return old_cred;
}

void ovl_revert_cached_creds(const struct cred* old) {
	validate_creds(old);
	rcu_assign_pointer(current->cred, (struct cred *)old);
}
...
int ovl_nlink_start(struct dentry *dentry)
{
	...
	old_cred = ovl_override_cached_creds(dentry->d_sb);
	/*
	...
	ovl_revert_cached_creds(old_cred);
